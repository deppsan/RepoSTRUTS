...
/*
 * Sugerencia del Estilo de errores.
   .errorField{
 ...
 *		nombre de la clase
 *		mensaje de error peque�o que se situa al lado dle campo input
...
var strError= [
	...
	['jqv-alphanumeric',
		'solo alfanumericos',
		'S�lo se aceptan letras y n�meros.',
		/^([a-z0-9,]+)?$/gi
	],
	['jqv-alphaextra',
		'solo alfanumericos',
		'S�lo se aceptan letras, n�meros y chars especiales.',
		/^([a-z\xE1\xE9\xED\xF3\xFA\xFC\xF1\s\d\.:;,]+)?$/gi
	],
	['jqv-numeric',
		'solo n�meros',
		'S�lo se aceptan n�meros enteros.',
		/^(\d+)?$/g
	],
	['jqv-percent',
		'formato porcentaje',
	...
	['jqv-float',
		'solo flotantes',
		'Solo se aceptan n�meros con/sin decimales.',
		/^((\d+)?((\.)(\d+))?)?$/g
	],
	['jqv-money',
		'solo dinero',
	...
	['jqv-email',
		'solo e-mail',
		'Debe contener una direcci�n de correo electr�nico estandar.',
		/^([\w\._]+@[\w\.]+\.[\w]{2,3})?$/gi
	],
	['jqv-phone',
		'Formato tel�fono incorrecto',
		'S�lo se aceptan 7, 8 o 10 d�gitos para t�lefonos fijos.',
		/^(\d{7}|\d{8}|\d{10}|)?$/
	],
	['jqv-cellphone',
		'formato tel�fono incorrecto',
		'T�lefono celular debe ser de 10 d�gitos.',
		/^(\d{10})?$/
	],
	['jqv-curp',
		'formato incorrecto',
	...
	['jqv-rfc',
		'solo RFC',
		'Formato no corresponde a RFC para Personas F�sica � Moral.',
		/^(([A-Za-z]{3,4})(\d{2})(0[1-9]|1[012])(0[1-9]|[12][0-9]|3[01])(\w{3}))?$/gi
	],
	['jqv-rfc-fisica',
		'solo RFC fisica',
		'Formato incorrecto para RFC de Persona F�sica.',
	...
	['jqv-date-mx',
		'fecha incorrecta',
		'Debe indicar una fecha v�lida en formato dd/mm/aaaa',
		/^(\d{2}\/\d{2}\/\d{4})?$/gi
	],
	['jqv-month',
		'Mes incorrecto',
		'Debes indicar un mes v�lido',
		/^(0[1-9]|1[01])?$/
	]
];
...
//    };
    ...
    //este handler maneja las nuevas clases de validacion automatica de los botones
    //Handler que maneja el boton Validador de todo con alerta
    $(document).on('click','.jqv-validate-all',function(e){
...
//TODO PROBAR FUNCIONAMIENTO
	...
	if (extra && ("áéíóúñ .:;,'").indexOf(character) > -1) 
		resultado = true;
	return resultado;
}
